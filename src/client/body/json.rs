...
use crate::{
    error::{decoding_error, json_serialization_error},
    serde::{deserialize_json, serialize},
};
...
where
    ...
    S: AsRef<[u8]>,
{
    let json = from_slice::<Json, _>(source).map_err(|err| decoding_error(ruby, err))?;
    serialize(ruby, &json)
}
...
impl TryConvert for Json {
    fn try_convert(value: Value) -> Result<Self, Error> {
        ...
        deserialize_json(&ruby, value)
            .map(Self)
            .map_err(|error| json_serialization_error(&ruby, error.to_string()))
    }
}
...
mod tests {
    ...
    fn preserves_number_precision_and_object_order() {
        ...
        let json = from_slice::<Json, _>(source).unwrap();
