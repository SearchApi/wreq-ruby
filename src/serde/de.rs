...
use number_deserializer::NumberDeserializer;
use variant_deserializer::VariantDeserializer;

/// Conversion behavior selected for a deserializer tree.
#[derive(Clone, Copy)]
pub(super) enum Mode {
    Generic,
    Json,
...
/// Deserialize one Ruby value into any Serde type.
pub(super) fn deserialize<'de, Output>(ruby: &Ruby, value: Value) -> Result<Output, Error>
where
    Output: Deserialize<'de>,
{
    Output::deserialize(Deserializer::new(ruby, value))
}

/// Deserialize one Ruby value into a `serde_json::Value`.
pub(super) fn deserialize_json(ruby: &Ruby, value: Value) -> Result<JsonValue, Error> {
    JsonValue::deserialize(Deserializer::new_json(ruby, value))
}
...
impl<'ruby> Deserializer<'ruby> {
    ...
    fn new_json(ruby: &'ruby Ruby, value: Value) -> Self {
