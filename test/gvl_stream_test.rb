...
class GvlStreamingTest < Minitest::Test
  ...
  def test_chunks_yields_string_chunks
  ...
  # =========================================================================
  # GVL concurrency — other Ruby threads can run during streaming
  # =========================================================================
  ...
  # =========================================================================
  # Thread interruption during streaming
  # =========================================================================

  def test_thread_interrupt_during_streaming
    ...
    assert killed, "Streaming should be interruptible via Thread.kill"
  end
  ...
  def test_chunks_propagates_streaming_errors
    ...
    begin
      resp.chunks do |_chunk|
        # Just consume chunks — the timeout should fire mid-stream
      end
    rescue => e
      error_raised = true
      assert(
  ...
  def test_exception_in_block_propagates
    ...
    begin
      resp.chunks do |chunk|
        ...
        if chunks_before_error == 2
          raise RuntimeError, "intentional error in block"
        end
      end
